...
namespace {
double SignedTetVolume(const Eigen::VectorXd &x, const Eigen::Vector4i &tet)
{
	Eigen::Vector3d p0 = x.segment<3>(3 * tet[0]);
	Eigen::Vector3d p1 = x.segment<3>(3 * tet[1]);
	Eigen::Vector3d p2 = x.segment<3>(3 * tet[2]);
	Eigen::Vector3d p3 = x.segment<3>(3 * tet[3]);
...
{
	VolumeStats stats{};
	stats.rest_total_volume = mRestVolumeStats.rest_total_volume;
	stats.min_tet_ratio = 1e10;
	stats.max_tet_ratio = 0.0;
	const auto &tets = mMesh->GetTetrahedrons();
	for (size_t i = 0; i < tets.size(); ++i)
	{
		double signed_vol = SignedTetVolume(x, tets[i]);
		if (signed_vol < 0.0)
			stats.inverted_tets++;
		stats.total_volume += signed_vol;
		double rest = mRestTetVolumes[i];
		if (std::abs(rest) > 1e-15)
		{
			double ratio = std::abs(signed_vol) / std::abs(rest);
			stats.min_tet_ratio = std::min(stats.min_tet_ratio, ratio);
			stats.max_tet_ratio = std::max(stats.max_tet_ratio, ratio);
		}
	}
	if (stats.rest_total_volume > 1e-15)
		stats.volume_ratio = stats.total_volume / stats.rest_total_volume;
	if (stats.min_tet_ratio > 1e9)
		stats.min_tet_ratio = 1.0;
	return stats;
}
...
void Worm::Initialize(FEM::World *world, bool load_driven_signals)
{
	std::cout << "  Init Worm...";
	...
	mRestTetVolumes.clear();
	mRestVolumeStats = {};
	for (const auto &tet : tetrahedras)
	{
		double vol = SignedTetVolume(v, tet);
		mRestTetVolumes.push_back(vol);
		mRestVolumeStats.rest_total_volume += vol;
	}
	mRestVolumeStats.total_volume = mRestVolumeStats.rest_total_volume;
	mRestVolumeStats.volume_ratio = 1.0;
	mRestVolumeStats.min_tet_ratio = 1.0;
	mRestVolumeStats.max_tet_ratio = 1.0;
	mRestVolumeStats.inverted_tets = 0;

	if (load_driven_signals)
	{
	...
	while (!ifs.eof())
	{
		...
		else if (!index.compare("l"))
		{
			if (bNewFiber)
			{
				...
				{
					std::cout << "  Skip Muscle " << muscle_name << ": not enough points." << std::endl;
					point_list.clear();
					bNewFiber = false;
					continue;
				}

				int muscle_num_sampling = std::min(num_sampling, static_cast<int>(point_list.size()) - 1);
				mMuscles.push_back(new Muscle(muscle_num_sampling, point_list));
				...
				mTotalMuscleSegments += muscle_num_sampling;
