...
{
	double total_volume;
	double rest_total_volume;
	double volume_ratio;
	double min_tet_ratio;
	double max_tet_ratio;
	int inverted_tets;
};
...
public:
	...
	void Initialize(FEM::World* world, bool load_driven_signals = true);
	...
	Eigen::Vector3d GetMeshBBMin() const { return mMesh->GetBBMin(); }
	Eigen::Vector3d GetMeshBBMax() const { return mMesh->GetBBMax(); }
	VolumeStats ComputeVolumeStats(const Eigen::VectorXd& x) const;
	const VolumeStats& GetRestVolumeStats() const { return mRestVolumeStats; }
...
private:
	...
	std::vector<double>						mRestTetVolumes;
	VolumeStats								mRestVolumeStats;
