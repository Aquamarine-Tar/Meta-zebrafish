...
SimulationWindow::SimulationWindow()
	: GLWindow(), mCurFrame(0), mTotalFrame(0), mElapsedTime(0.0), mFocusWormId(0), mPlay(false), mController(MetaController::MC_Default_Key),
	  ...
	  mbDrawWorld(true), mbDraw2DGraph(true), mbDrawAxis(false), mbDrawPath(true), mbDrawReferenceTets(false),
	  mbDrawFluidParticles(false), mDrawFood(true), mStartScreenShot(false)
{
	mDisplayTimeout = 33;
	...
	// Worm body is small; use a closer default camera so it is visible at startup.
	mCamera->SetCamera(Eigen::Vector3d(0, 0.0, 0) /*lookat*/, Eigen::Vector3d(0.32, 0.0, 0.0) /*eye*/, Eigen::Vector3d(0, 1, 0) /*up*/);
	...
	try
	{
		...
		// C302 imports WormInEnv only when that controller is selected.
...
#else
		std::cout << "  C302/eworm support is disabled. Rebuild with METAWORM_ENABLE_EWORM=ON to use it." << std::endl;
		mController = MetaController::MC_SineMuscle;
		AddWorm(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, -1), Eigen::Vector4d(0.79, 0.79, 0.79, 0.4));
		mActions.resize(mWorms[0]->GetActions().size());
		mActions.setZero();
#endif
	}
	else if (network_name == std::string("sin") || network_name == std::string("sine"))
	{
		mController = MetaController::MC_SineMuscle;
		AddWorm(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, -1), Eigen::Vector4d(0.79, 0.79, 0.79, 0.4));
		mActions.resize(mWorms[0]->GetActions().size());
		mActions.setZero();
		std::cout << "  Controller: external sine muscle activation" << std::endl;
	}
	else
	{
		AddWorm(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, -1), Eigen::Vector4d(0.79, 0.79, 0.79, 0.4));
...
Eigen::VectorXd SimulationWindow::GetSineMuscleActivations(Environment* worm) const
{
	...
	const double time = static_cast<double>(worm->GetPhase()) / static_cast<double>(worm->GetControlHz());
	const auto& muscles = worm->GetCreature()->GetMuscles();
	Eigen::VectorXd activations = Eigen::VectorXd::Zero(worm->GetMuscleActivationSize());

	int idx = 0;
	for (int muscleIdx = 0; muscleIdx < muscles.size(); ++muscleIdx)
	{
		int sampleCount = muscles[muscleIdx]->GetNumSampling();
		double musclePhase = (muscleIdx % 2 == 0) ? 0.0 : pi;

		for (int sampleIdx = 0; sampleIdx < sampleCount; ++sampleIdx)
		{
			double bodyPhase = 2.0 * pi * spatialCycles * static_cast<double>(sampleIdx) / static_cast<double>(sampleCount);
			double signal = offset + amplitude * std::sin(2.0 * pi * temporalFrequency * time - bodyPhase + musclePhase);
...
void SimulationWindow::Display()
{
	...
	if (mFocusWormId < mWorms.size())
	{
		...
		PeridynoBridge* fluidBridge = mWorms[mFocusWormId]->GetFluidBridge();
		const bool hasFluidBridge = fluidBridge && fluidBridge->IsInitialized();
		if (hasFluidBridge && (mbDraw2DGraph || mbDrawFluidParticles))
			mFluidParticleVisualizer.Update(fluidBridge);
		...
		if (mbDraw2DGraph)
		{
			...
			if (hasFluidBridge)
			{
				const double fluidHudY = init_y - (muscles.size() + 1) * 1.2 * height;
				GUI::DrawFluidVelocityHud(
					init_x, fluidHudY, length, height,
					Eigen::Vector3d(0.22, 0.55, 0.85),
					mFluidParticleVisualizer.GetSpeedHistory(),
					mFluidParticleVisualizer.GetVelocityMax());
			}
		}
	}
...
void SimulationWindow::DisplayWorm(Environment *mEnvironment, std::deque<Eigen::Vector3d> &mTrajectory, std::deque<Eigen::Vector3d> &mRefCoordTrajectory, Eigen::Vector4d color, int wormIdx)
{
	...
	//-------------------FSI Fluid Particles-------------------------
	if (mbDrawFluidParticles && wormIdx == mFocusWormId && mFluidParticleVisualizer.GetParticleCount() > 0)
	{
		glColor4f(1.0f, 1.0f, 1.0f, 0.85f);
		mFluidParticleVisualizer.Draw();
	}
...
void SimulationWindow::Keyboard(unsigned char key, int x, int y)
{
	...
	case 'u':
	{
		mbDrawFluidParticles = !mbDrawFluidParticles;
		std::cout << (mbDrawFluidParticles ? "Show" : "Hide") << " FSI fluid particles (press 'u')." << std::endl;
		break;
	}
	case 'v':
	{
...
void SimulationWindow::Timer(int value)
{
	...
	if (mPlay)
	{
		...
		break;
		case MetaController::MC_SineMuscle:
		{
			for (auto worm : mWorms)
			{
				unsigned int cur_phase = worm->GetPhase();
				Eigen::VectorXd activations = GetSineMuscleActivations(worm);
				worm->GetCreature()->SetActivationLevelsDirectly(activations);
...
{
	auto worm = new Environment("data/fish.meta", false);
	worm->Reset();
