...
void GUI::DrawFluidVelocityHud(const double& x, const double& y, const double& length, const double& height,
	const Eigen::Vector3d& color, const std::deque<float>& speedHistory, float speedMax)
{
	const float vmax = std::max(speedMax, 1e-6f);
	const int n = std::max(2, (int)speedHistory.size());
	...
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glBegin(GL_POLYGON);
	glColor4f(0.85f, 0.85f, 0.85f, 0.7f);
	glVertex2d(x, y);
	...
	const double base = y + 0.5 * height;
	const int histN = (int)speedHistory.size();
	if (histN >= 2)
	{
		for (int i = 0; i < histN - 1; ++i)
		{
			const double x1 = x + length / (histN - 1) * i;
			const double x2 = x + length / (histN - 1) * (i + 1);
			const double norm1 = std::min(1.0, (double)(speedHistory[i] / vmax));
			const double norm2 = std::min(1.0, (double)(speedHistory[i + 1] / vmax));
			double y1 = base + norm1 * 0.5 * height;
			double y2 = base + norm2 * 0.5 * height;
			if (y1 < y) y1 = y;
			if (y2 < y) y2 = y;
	...
	{
		const double norm = std::min(1.0, (double)(speedHistory[0] / vmax));
		double y1 = base + norm * 0.5 * height;
		if (y1 < y) y1 = y;
		glColor4f(color[0], color[1], color[2], 0.7f);
		glBegin(GL_POLYGON);
		glVertex2d(x, base);
		glVertex2d(x, y1);
		glVertex2d(x + length, y1);
		glVertex2d(x + length, base);
		glVertex2d(x, base);
		glEnd();
	}
